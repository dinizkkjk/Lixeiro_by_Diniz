...
emp = Tunnel.getInterface("hypex_lixeiro")
    ...
    while true do
        ...
        if not trabalhando then 
            ...
            if DistCentral <= 15 then
                sleep = 4
                --DrawMarker(21, central[1], central[2], central[3], 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 250,255,150,150, 1, 0, 0, 1)
        ...
        elseif trabalhando and not CarroSpawnado then
            ...
            if DistGaragem <= 15 then
                ...
                if DistGaragem <= 1.5 then
                    sleep = 4
                    DrawText3D(garagem[1], garagem[2], garagem[3], '~r~[E] ~w~PARA SOLICITAR CAMINHÃO')
                    if IsControlJustPressed(0,38) then
                        CarroSpawnado = true
                        SpawnStockade()
                    end
